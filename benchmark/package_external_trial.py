...
def render_readme(trial: dict[str, Any], validation_detail: str, artifact_manifest: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        f"- claim_status: `{artifact_manifest['claim_status']}`",
        f"- evidence_scope: `{artifact_manifest['evidence_scope']}`",
        f"- final_production_signoff: `{artifact_manifest['final_production_signoff']}`",
        ...
        f"- readiness_claim_status: `{readiness.get('claim_status')}`",
        f"- readiness_evidence_scope: `{readiness.get('evidence_scope')}`",
        f"- readiness_final_production_signoff: `{readiness.get('final_production_signoff')}`",
        ...
        f"- readiness_workspace: `{readiness.get('workspace')}`",
        f"- readiness_manifest: `{readiness.get('manifest')}`",
        ...
        "This package is one required production-gate input, not a final production signoff by itself.",
        "",
        "## Reviewer Entry Point",
        "",
        "Chinese-community reviewers can use `README.zh-CN.md` as a first-class review entrypoint for this same evidence package.",
        "Before final signoff, reviewers must still re-check this package with `verify_external_evidence_package.py`, then bind the saved package verifier report to the final production wrapper together with the external trial report, saved trial reviewer report, stable release tag, and saved stable-release verifier report.",
        "",
        "```text",
        validation_detail,
        "```",
...
def render_readme_zh(trial: dict[str, Any], validation_detail: str, artifact_manifest: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "这个 package 是 production gate 的一个必需输入，但它本身不是最终生产签核。",
        "",
        "## 中文 reviewer 入口",
        "",
        "中文社区 reviewer 可以把 `README.zh-CN.md` 作为这个 evidence package 的一等复核入口。",
        "最终签核前，reviewer 仍必须用 `verify_external_evidence_package.py` 复核这个 package，然后把 saved package verifier report 与外部 trial report、saved trial reviewer report、稳定 release tag、saved stable-release verifier report 一起绑定到 final production wrapper。",
        "",
        "```text",
        validation_detail,
        "```",
...
def create_package(
    ...
    artifact_manifest = {
        ...
        "trial_claim_status": trial.get("claim_status"),
        "trial_evidence_scope": trial.get("evidence_scope"),
        "trial_final_production_signoff": trial.get("final_production_signoff"),
