...
def render_readme_zh(plan: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "`audit_production_evidence` 生成的 saved production evidence audit 也不是最终生产签核：它必须保持 `claim_status=NOT_PRODUCTION_CLAIM`、`evidence_scope=PRODUCTION_EVIDENCE_READINESS_AUDIT`、`final_production_signoff=false`。它必须在 `final_production_gate` 运行前用 `--verify-report-files --require-ready` 重新复核；如果同时存在两份 saved external reviewer reports，它还必须保留 saved trial verifier、saved package verifier 和 `Verify saved external L4 reviewer report pair` gate entries。",
