...
def render_readme(plan: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "`verify_local_evidence_preflight` rehashes the source trial JSON, packaged trial JSON, package `artifact_manifest.json`, package `readiness.json`, zip, checksum, and reviewer reports; it also recomputes the source/package trial claim-scope labels, package-manifest package/source-trial scope labels, and readiness `package_name` before PASS can be accepted.",
...
def render_readme_zh(plan: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "`verify_local_evidence_preflight` 会重新 hash source trial JSON、package 内 trial JSON、package `artifact_manifest.json`、package `readiness.json`、zip、checksum 和 reviewer reports；它还会重新计算 source/package trial claim-scope labels、package manifest 的 package/source-trial scope labels，以及 readiness `package_name`，全部通过后才可接受 PASS。",
