...
def render_readme(plan: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "`run_trial` re-verifies the saved READY report and refuses to execute if its manifest or workspace does not match the current trial manifest and `base_dir`.",
        "`verify_local_evidence_preflight` rehashes the source trial JSON, packaged trial JSON, package `artifact_manifest.json`, package `readiness.json`, zip, checksum, and reviewer reports; it also requires required input-artifact summaries to remain `exists=true`, requires metadata-bound saved reviewer reports to keep matching gate entries and hash summaries, and recomputes the source/package trial claim-scope labels, package-manifest package/source-trial scope labels, and readiness `package_name`, workspace, and manifest before PASS can be accepted.",
...
def render_readme_zh(plan: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "`run_trial` 会重新复核 saved READY report，并且在 report 的 manifest 或 workspace 与当前 trial manifest 和 `base_dir` 不一致时拒绝执行。",
        "`verify_local_evidence_preflight` 会重新 hash source trial JSON、package 内 trial JSON、package `artifact_manifest.json`、package `readiness.json`、zip、checksum 和 reviewer reports；它还要求必需的 input-artifact summaries 保持 `exists=true`，要求 metadata 绑定的 saved reviewer reports 保留对应 gate entries 和 hash summaries，并重新计算 source/package trial claim-scope labels、package manifest 的 package/source-trial scope labels，以及 readiness `package_name`、workspace 和 manifest，全部通过后才可接受 PASS。",
