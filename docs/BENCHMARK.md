...
The package command refuses invalid trial reports and writes a directory, zip, SHA-256 file, rendered manifest snapshot, external evidence JSON, artifact manifest, and copied artifacts. Release gate checks that the zip contains exactly every required package file and declared artifact, with no missing or extra entries, and that the checksum file contains a valid digest for the package zip filename.
