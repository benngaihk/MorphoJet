...
- External evidence gate: `--require-external-evidence` requires lab/workflow owner, dataset source, downstream workflow, execution environment, acceptance criteria, `manual_csv_editing=false`, a rendered manifest snapshot, and one SHA-256/size provenance entry for each listed artifact for real external trials.
