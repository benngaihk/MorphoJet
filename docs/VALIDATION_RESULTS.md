...
Saved release-gate JSON reports can be re-checked with `benchmark/verify_release_gate_report.py`, which requires the top-level summary fields to match `production_claim_audit`, validates metadata and gate entry schemas, requires the expected production-audit check list, rejects production PASS reports missing required production gates or final production metadata flags/paths/stable-release identity, and can require both report PASS and production-claim PASS for final signoff.
