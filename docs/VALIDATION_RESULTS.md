...
External L4 evidence package validation now requires the README to preserve trial/signoff fields, requires absolute trial JSON/root source metadata, source trial JSON size/SHA-256, review-file size/SHA-256 entries, release-gate input matching, and unique package paths in the artifact manifest, requires the package zip to contain exactly the required review files and manifest-declared artifacts, rejects duplicate, missing, and extra zip entries, checks zip entry bytes against the package directory, and validates the zip checksum digest format plus target filename.
