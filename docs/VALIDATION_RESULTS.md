...
External L4 evidence package validation now requires the package zip to contain exactly the required review files and manifest-declared artifacts, rejecting both missing and extra entries, and validates the zip checksum digest format plus target filename.
