...
External L4 evidence package validation now requires unique package paths in the artifact manifest, requires the package zip to contain exactly the required review files and manifest-declared artifacts, rejects both missing and extra entries, and validates the zip checksum digest format plus target filename.
