...
After external L4 local preflight and after saving GitHub workflow and stable-release verifier reports, use `benchmark/audit_production_evidence.py` as a lightweight final-input inventory. It writes JSON/Markdown reports labeled `claim_status=NOT_PRODUCTION_CLAIM`, `evidence_scope=PRODUCTION_EVIDENCE_READINESS_AUDIT`, and `final_production_signoff=false`, maps the final evidence inputs to the same blocker names as release gate, and can re-check a saved report with `--verify-report path/to/production-evidence-audit.json --require-ready`. This audit is useful before the final wrapper, but it is not a production signoff and does not replace `benchmark/run_production_gate.py` plus saved final `verify_release_gate_report.py` verification.
