...
The saved report verifier rejects reports whose `metadata.generated_at_utc` is not UTC, whose top-level `status` conflicts with recorded gate statuses, or whose recorded `metadata.require_production_claim` / `production_claim_status` combination is inconsistent.
