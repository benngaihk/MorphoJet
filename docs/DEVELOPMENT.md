...
After external L4 local preflight and after saving GitHub workflow and stable-release verifier reports, use `benchmark/audit_production_evidence.py` as a lightweight final-input inventory. It writes JSON/Markdown reports labeled `claim_status=NOT_PRODUCTION_CLAIM`, `evidence_scope=PRODUCTION_EVIDENCE_READINESS_AUDIT`, and `final_production_signoff=false`, maps the final evidence inputs to the same blocker names as release gate, and records `input_files` size/SHA-256 summaries for the external trial JSON plus each supplied saved final-input reviewer, workflow, and stable-release report. When both saved external reviewer reports are supplied, the audit report must retain the saved trial verifier, saved package verifier, and reviewer-report pair gate entries before `external_l4_saved_reviewer_reports` can pass. It can re-check a saved report with `--verify-report path/to/production-evidence-audit.json --verify-report-files --require-ready`. The saved audit verifier treats `--require-ready` as signoff mode, so it fails unless file recheck is enabled, the report's git commit matches the current checkout, the recorded argv/input paths remain absolute and bound to the verified report, recorded input-file hashes still match the current files, required saved-reviewer gate entries remain present, and recomputed audit gate statuses still match. The final wrapper now also requires `--production-evidence-audit-report path/to/production-evidence-audit.json` and reruns that saved-audit recheck before invoking the release gate. This audit is useful before the final wrapper, but it is not a production signoff and does not replace `benchmark/run_production_gate.py` plus saved final `verify_release_gate_report.py` verification.
