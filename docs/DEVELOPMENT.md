...
The external trial gate requires `status=PASS`, all trial steps passing, a non-empty artifact list whose files exist and are non-empty under `--external-trial-root`, matching `artifact_provenance` SHA-256/size entries, filled external evidence fields with no `REPLACE_WITH` placeholders, and `manual_csv_editing=false`.
