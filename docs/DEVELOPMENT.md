...
Workspaces prepared by `benchmark/prepare_external_l4_trial.py` include this saved local-preflight verifier as the final generated command after `local_evidence_preflight`, so reviewers rehash the saved input files, rerun the local external L4 gates, recompute the package-readiness package-name summary, and require PASS before treating the preflight package as ready for the later stable-release gate.
