...
1. Prepare an external trial workspace with `benchmark/prepare_external_l4_trial.py --workspace path/to/external-trial`, which refuses stale trial, reviewer, preflight, or package outputs before writing the scaffold; fill the copied manifest's required external evidence block, place the real batch handoff files, run the generated readiness audit with `benchmark/check_external_l4_readiness.py` so the input CSV schemas match the declared object set and channels and manifest-declared trial outputs plus planned reviewer/preflight reports do not already exist, run the generated validation and `benchmark/run_handoff_trial.py --require-external-evidence` commands, validate the resulting report with release gate, and package it with `benchmark/package_external_trial.py` for review/signoff.
...
This is the single command intended to produce the final production-claim report. It requires a stable non-RC tag, checks that the external evidence paths and supplied reviewer verification reports exist before an actual run, re-checks saved reviewer reports when supplied, requires saved package reviewer reports to be bound to a source `trial_json`, requires saved trial/package reviewer reports to match the current external trial JSON, trial root, and evidence package directory inputs, requires the saved GitHub release verification report to be a stable PASS report for the same final tag and the `benngaihk/MorphoJet` repo when supplied, passes those reviewer-report checks into the final release-gate JSON/Markdown, and delegates to `benchmark/release_gate.py --require-production-claim`, so production remains incomplete until that command passes with real external evidence and a live stable release verification. Direct release-gate saved reviewer-report gates also bind saved external trial/package reports to the current external evidence inputs, re-check saved GitHub release reports with `--verify-git-commit`, bind saved GitHub reports to the supplied live release tag when `--verify-github-release` is present, and reject report output paths that would overwrite external evidence inputs, package files/artifacts, reviewer reports, or the GitHub release verifier report.
...
Use `benchmark/run_production_gate.py --verify-local-evidence-preflight-report path/to/local-evidence-preflight.json` to re-check the saved local evidence-preflight report's schema, reachable git commit, claim-scope labels, final-evidence rejection flag, non-empty metadata evidence paths, unique input artifact paths, metadata-to-input-artifact path bindings, gate entry shape, unique gate names, and consistency between top-level status and recorded gate statuses during review. Add `--verify-local-evidence-preflight-files` when the referenced evidence files are available so the recorded input file sizes and SHA-256 hashes are recomputed. Add `--verify-local-evidence-preflight-gates` to rerun recorded external L4 trial, package, and saved reviewer-report gates from report metadata. Add `--require-local-evidence-preflight-pass` for review/signoff.

Use `benchmark/verify_external_trial_report.py --verify-report path/to/trial-verification.json --verify-report-files --require-report-pass` during signoff to re-check the saved external L4 trial reviewer report. The saved report records canonical verifier `argv`, source trial JSON and resolved artifact file size/SHA-256 summaries, requires those summary paths to remain bound to the saved top-level trial JSON/trial root inputs, requires `--json-out` bound to the saved report path under review, rejects reviewer report outputs that would overwrite or create files inside the source trial JSON or declared artifacts, and file recheck recomputes those summaries plus the external trial validation gate. The production wrapper additionally rejects a saved trial reviewer report whose saved trial JSON or trial root differs from the current wrapper inputs.

Use `benchmark/verify_external_evidence_package.py --verify-report path/to/package-verification.json --verify-report-files --require-report-pass --require-trial-json` during signoff to re-check the saved external L4 evidence-package reviewer report. The saved report must be bound to the source trial JSON, rejects unexpected saved input-file entries, records canonical verifier `argv` plus size/SHA-256 summaries for that source trial JSON, the package review files, package zip, and checksum file, requires `--json-out` bound to the saved report path under review, rejects reviewer report outputs that would overwrite or create files inside source/package evidence files, and requires those summary paths to remain bound to the saved top-level package/source-trial inputs; file recheck recomputes those summaries and the package validation gate. The package validation gate also requires the package artifact manifest to preserve canonical packager `argv` matching the source trial JSON, trial root, output directory, and package name. The production wrapper additionally rejects a saved package reviewer report whose saved package directory or source trial JSON differs from the current wrapper inputs.

Use `benchmark/verify_github_release.py --verify-report path/to/github-release/verification.json --verify-report-files --require-report-pass --require-stable-report --verify-git-commit --expect-tag v0.1.0` to re-check a saved stable-release verification report during signoff, or pass the same report to `run_production_gate.py --github-release-verification-report` so the final release-gate report records the saved-report gate. This recomputes downloaded asset names, archive SHA-256 values, checksum file contents, and recorded GitHub asset metadata identity/state/timestamp/size/digest fields from the release output directory, rejects saved PASS reports for draft releases, requires saved reports to bind release and asset URLs to the saved repo/tag/asset names, requires `assets.expected` to match the required release assets for the saved tag, requires GitHub release ID/API identity plus release author, target commit-ish, immutable state, and release timestamps, requires unique GitHub asset IDs and API asset URLs bound to the saved repo, requires canonical verifier `argv` to bind the saved tag, repo, output directory, stable/prerelease expectation, and saved report path, requires archive SHA-256 summaries to match the corresponding GitHub asset digest, requires saved reports to bind the full 40-character release commit to the 12-character `morphojet doctor` commit prefix, and confirms the expected tag resolves to the saved commit in the current git checkout; the production wrapper additionally rejects a saved GitHub release report whose repo is not `benngaihk/MorphoJet`. This saved report does not replace `--verify-github-release`.
