...
If the external L4 evidence is ready before the stable release, run the same wrapper with `--local-evidence-preflight-only` to validate only the external trial report and evidence package and write a local evidence-preflight JSON/Markdown report. That report is machine-labeled `NOT_PRODUCTION_CLAIM` and lists the skipped final checks. Passing that preflight reduces L4 packaging risk, but it does not satisfy the stable-release or final production-claim gates.
