...
Final production wrapper 也有 saved reviewer/verifier 输入的回归覆盖：final command，包括 `--dry-run`，必须同时提供 `--external-trial-verification-report`、`--external-evidence-package-verification-report` 和 `--github-release-verification-report`；dry-run 只跳过文件存在性检查，local-preflight mode 仍是明确的非最终例外。
