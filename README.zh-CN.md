...
Release gate 会运行 `benchmark/validate_claim_language.py`，检查英文 README 指向 `README.zh-CN.md`，并检查中文 README 保留中文社区验证入口、外部 L4 试验、最终生产门禁和当前里程碑状态。它也会要求中英文 README 同时保留关键机器锚点、`Verify saved external L4 reviewer report pair`、stable-release saved-report 签核 flags（`--require-stable-report`、`--expect-tag`、`--expect-repo`、`--expect-commit`）、release archive 的 `package_files` / `matches_source` 摘要和 final wrapper saved-report flags。这样中文 reviewer 可以按同一条 evidence chain 复核项目，而不是依赖一条更弱或过期的并行说明。
...
本地 archive verifier 会检查 checksum、tar 安全解包、`morphojet doctor`，以及 `README.zh-CN.md` 等必需 package files。它还会写入 `package_files` 摘要，要求包内 `README.md`、`README.zh-CN.md` 和 `LICENSE` 对当前仓库源文件都是 `matches_source=true`，防止稳定 release 包携带旧版中文 README。
...
GitHub release verifier 会检查 tag 身份、release URL、GitHub release ID/API 身份、作者、target commit-ish、UTC 时间戳、draft/prerelease/immutable 状态、稳定 semver tag、发布资产集合、下载文件 checksum、每个 archive 的安全解包和内容，并要求 saved release API URL 与记录的 release database ID 匹配、当前机器兼容的 archive 能通过 `morphojet doctor` 且 commit 前缀匹配、所有 archive entry 的顶层 `package_files` 中 `README.md`、`README.zh-CN.md` 和 `LICENSE` 都对当前仓库源文件保持 `matches_source=true`。使用 `--verify-report-files` 复核 saved GitHub release report 时，verifier 还会重新解包每个已保存的 archive，重新计算这些 package-file 摘要并与 saved JSON 中的稳定字段比较，避免旧版中文 README 或被篡改的 `package_files` 摘要只靠保存报告混入最终签核链。Direct release gate 的 live GitHub release verification 会绑定生产仓库 `benngaihk/MorphoJet`；生成的外部 L4 plan 会让 live stable-release verifier 用 `--expect-commit <trial_plan git_commit>` 绑定计划 commit，并让 saved stable-release report 复核也用同一个 `--expect-commit`，防止稳定 tag、下载 archive、`doctor` commit prefix、saved release report、每个包内双语 README 和最终 workflow evidence 指向不同提交。Saved GitHub release verifier report 会标记 `claim_status=NOT_PRODUCTION_CLAIM`、`evidence_scope=GITHUB_STABLE_RELEASE_VERIFICATION`、`final_production_signoff=false`；复核 saved report 时可用 `--expect-repo benngaihk/MorphoJet` 和 `--expect-commit <final-commit>` 防止其他仓库或旧提交的 release 报告进入签核链。
...
这个 audit 报告仍是非最终证据，会写入 `schema_version=2`、`claim_status=NOT_PRODUCTION_CLAIM`、`evidence_scope=PRODUCTION_EVIDENCE_READINESS_AUDIT`、`final_production_signoff=false`、`production_claim_status=PASS|INCOMPLETE`，并沿用 release gate 的 production blocker 名称。它还会把外部 trial JSON、saved trial/package reviewer reports、saved GitHub workflow report 和 saved stable-release verifier report 写入 `input_files`，记录绝对路径、存在性、size 和 SHA-256；也会把 `external_trial_root` 和 `external_evidence_package_dir` 写入 `input_artifacts`，记录绝对路径、是否存在、是否仍是目录。生成的外部 L4 工作区中英文 README 也会保留这些 audit v2 和目录输入锚点，所以 saved plan file verification 会拦住被改弱的外部 reviewer 指引。Markdown 会渲染输入文件和输入目录摘要，方便中文社区 reviewer 不打开 JSON 也能复核最终输入是否被替换。只要同时提供 trial/package 两份 saved external reviewer reports，audit 生成和 saved audit 复核都会要求三条 saved-reviewer gates 保留：saved trial verifier、saved package verifier 和 `Verify saved external L4 reviewer report pair`。最终 wrapper 前用 `python3 benchmark/audit_production_evidence.py --verify-report path/to/production-evidence-audit.json --verify-report-files --require-ready` 复核；最终 wrapper 也必须通过 `--production-evidence-audit-report` 消费同一份 saved audit，并在 wrapper 内再次用 `--verify-report-files --require-ready` 复核。它会按 saved metadata 路径重新运行 audit gates，并拒绝旧 commit、argv、输入路径、输入文件 hash、输入目录存在性或类型变化、saved-reviewer gate 被删除、saved gate 的 name/status/command 签名变化，或 final-wrapper-command 被篡改的 audit JSON。它只是 reviewer checklist 和路径绑定 guard，不能替代 `benchmark/run_production_gate.py`，也不能替代最终 saved `verify_release_gate_report.py --require-production-claim-pass --expect-missing-checks none` 签核。
...
live-run 复核会对短暂的 `gh run list` 失败进行重试，并为每个 workflow summary 记录 `query_attempts` 和 `query_max_attempts`；如果 GitHub CLI/API 仍不可用，它会记录可读的 `FAIL` summary，而不是只留下 traceback。
...
这个 wrapper 要求稳定非 RC tag；final command（包括 `--dry-run`）会 fail-fast 要求五份 saved final-input reports 都显式提供：`--external-trial-verification-report`、`--external-evidence-package-verification-report`、`--github-release-verification-report`、`--github-workflow-verification-report` 和 `--production-evidence-audit-report`。真实非 dry-run final run 会在调用 final gate 前检查外部 trial JSON、trial root、evidence package dir 和五份 saved verifier/audit reports 都存在，复核外部 trial/package reviewer 报告，用 `--verify-report-files --require-ready` 复核 saved production evidence audit，拒绝 final output 覆盖 package review files（包括 `README.md` 和 `README.zh-CN.md`），要求 saved GitHub release verifier report 是同一 final tag、`benngaihk/MorphoJet` repo 和当前 final commit 的 stable PASS 报告，并带有 PASS enforcement、stable-report、file recheck、git commit、expected tag、expected repo 和 expected commit 绑定，还要求每个 saved release archive entry 都保留顶层 `package_files`，且 `README.md`、`README.zh-CN.md` 和 `LICENSE` 都是 `matches_source=true`；同时要求 saved GitHub workflow report 绑定 `benngaihk/MorphoJet`、`main`、当前 git commit、`ci.yml` 和 `external-l4-rehearsal.yml`，再委托 `benchmark/release_gate.py --require-production-claim`。最终 release-gate JSON/Markdown 现在也会记录 `Verify saved production evidence audit report` gate 和 `production_evidence_readiness_audit` audit check，并要求 direct release gate 同样提供 `--production-evidence-audit-report`，避免 wrapper 预检和最终 saved report 脱节。如果 final release gate 通过，wrapper 会立刻复核 `production-claim.json`，要求 `--require-report-pass --require-clean-git-metadata --verify-git-commit --require-production-claim-pass --expect-missing-checks none`；生成的 trial plan 也保留同一条 final report verification 作为独立签核步骤。最终生产声明现在会把 saved trial/package reviewer reports、saved stable-release verifier report、saved GitHub Actions workflow verifier report 和 saved production evidence audit 作为独立审计项；saved GitHub release verifier report 自身仍标记 `claim_status=NOT_PRODUCTION_CLAIM`、`evidence_scope=GITHUB_STABLE_RELEASE_VERIFICATION`、`final_production_signoff=false`，saved workflow verifier report 也仍是 `claim_status=NOT_PRODUCTION_CLAIM`、`evidence_scope=GITHUB_ACTIONS_WORKFLOW_VERIFICATION`、`final_production_signoff=false`，saved production evidence audit 仍是 `evidence_scope=PRODUCTION_EVIDENCE_READINESS_AUDIT`，不是最终签核。direct release gate 的 live GitHub release verification、saved GitHub release report 复核和 saved workflow report 复核都会绑定 `benngaihk/MorphoJet` 和最终 commit/tag 上下文。GitHub release verifier 现在也把 `--require-stable-report` 当作签核模式参数；没有同时提供 `--require-report-pass`、`--verify-report-files`、`--verify-git-commit`、`--expect-tag`、`--expect-repo`、`--expect-commit` 和逐 archive package-file source matching 时会直接失败，避免 saved stable-release report 被弱复核、旧提交报告或旧版中文 README 替换。`--dry-run` 仍可用来查看 saved preflight verifier commands、组装后的 release-gate command 和 final-report verifier command，且不要求这些已提供的外部路径存在；`--local-evidence-preflight-only` 仍是较早的非最终路径，允许外部 saved reviewer reports 还未齐。它通过之前，项目不能宣称 production-ready。
...
- 当前 `main` 也已用 `benchmark/prepare_external_l4_trial.py --verify-plan-files --require-plan-files` 重新生成并复核外部 L4 workspace plan，确认中英文 trial README、saved reviewer reports、local preflight、production evidence audit、stable-release verifier、saved workflow verifier 和 final wrapper 命令仍绑定在同一条非最终 evidence chain 上；生成的中英文 workspace README 也会说明：当两份 saved external reviewer reports 都存在时，saved production evidence audit 必须保留 saved trial verifier、saved package verifier 和 `Verify saved external L4 reviewer report pair` gate entries。
