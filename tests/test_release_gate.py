...
class ReleaseGateTest(unittest.TestCase):
    ...
    def production_args(self, **overrides: object) -> Namespace:
        values = {
            ...
            "production_evidence_audit_report": None,
    ...
    def production_claim_args(self, **overrides: object) -> Namespace:
        values = {
            ...
            "production_evidence_audit_report": Path("production-evidence-audit.json"),
    ...
    def test_require_production_claim_rejects_every_partial_final_evidence_contract(self) -> None:
        ...
        for mask in range(1 << len(required_fields)):
            ...
            if "production_evidence_audit_report" in present:
                overrides["production_evidence_audit_report"] = Path("production-evidence-audit.json")
            for field in [
                "external_trial_json",
                "external_trial_root",
                "external_evidence_package_dir",
    ...
    def test_require_production_claim_rejects_unbound_saved_external_package_report(self) -> None:
        failures = release_gate.production_claim_contract_failures(
            self.production_args(
                require_production_claim=True,
                require_clean_git=True,
    ...
    def test_production_claim_audit_defaults_to_incomplete_without_l4_or_stable_release(self) -> None:
        ...
        self.assertEqual("MISSING", statuses["production_evidence_readiness_audit"])
        self.assertEqual(
            [
                "clean_git_worktree",
    ...
    def test_require_production_claim_fails_incomplete_audit(self) -> None:
        ...
        checklist = written_payload["production_claim_checklist"]
        self.assertEqual(
            [check["name"] for check in written_payload["production_claim_audit"]["checks"]],
            [row["check"] for row in checklist],
        )
        self.assertIn(
            {
                "check": "external_l4_workflow_trial",
                "status": "MISSING",
    ...
    def test_markdown_report_includes_production_claim_checklist(self) -> None:
        ...
        self.assertIn(
            ...
            "stable_github_release_saved_report, production_evidence_readiness_audit`.",
            markdown,
        )
        ...
        self.assertIn("| production_evidence_readiness_audit | MISSING |", markdown)
        self.assertIn("--verify-report-files --require-ready", markdown)
    ...
    def test_markdown_report_uses_json_production_claim_checklist(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            payload["production_claim_checklist"][0]["next_action"] = "Injected from JSON checklist."
            markdown = release_gate.render_markdown({**payload, "gates": gates}, root / "report.json")

        self.assertIn("Injected from JSON checklist.", markdown)
    ...
    def test_l3_provenance_compatible_path_allowlist(self) -> None:
        ...
        self.assertTrue(release_gate.is_l3_provenance_compatible_path(".github/workflows/cellbindb-l3.yml"))
        self.assertTrue(release_gate.is_l3_provenance_compatible_path(".github/workflows/external-l4-rehearsal.yml"))
        ...
        self.assertTrue(release_gate.is_l3_provenance_compatible_path("benchmark/audit_production_evidence.py"))
        self.assertTrue(release_gate.is_l3_provenance_compatible_path("benchmark/inspect_cellbindb_direct_masks.py"))
        ...
        self.assertTrue(release_gate.is_l3_provenance_compatible_path("benchmark/run_external_l4_rehearsal.py"))
        self.assertTrue(release_gate.is_l3_provenance_compatible_path("benchmark/run_production_gate.py"))
        self.assertTrue(release_gate.is_l3_provenance_compatible_path("benchmark/triage_oracle_candidates.py"))
        ...
        self.assertTrue(release_gate.is_l3_provenance_compatible_path("benchmark/verify_github_workflows.py"))
    ...
    def test_external_trial_path_allowlist(self) -> None:
        ...
        self.assertTrue(release_gate.is_external_trial_compatible_path(".github/workflows/cellbindb-l3.yml"))
        self.assertTrue(release_gate.is_external_trial_compatible_path(".github/workflows/external-l4-rehearsal.yml"))
        ...
        self.assertTrue(release_gate.is_external_trial_compatible_path("benchmark/audit_production_evidence.py"))
        self.assertTrue(release_gate.is_external_trial_compatible_path("benchmark/inspect_cellbindb_direct_masks.py"))
        ...
        self.assertTrue(release_gate.is_external_trial_compatible_path("benchmark/run_external_l4_rehearsal.py"))
        self.assertTrue(release_gate.is_external_trial_compatible_path("benchmark/run_production_gate.py"))
        self.assertTrue(release_gate.is_external_trial_compatible_path("benchmark/triage_oracle_candidates.py"))
        ...
        self.assertTrue(release_gate.is_external_trial_compatible_path("benchmark/verify_github_workflows.py"))
    ...
    def test_saved_external_trial_report_command_requires_file_recheck_and_pass(self) -> None:
        ...
        command = release_gate.saved_external_trial_report_command(report, expected_commit="c" * 40)

        self.assertEqual(str(report.resolve(strict=False)), command[command.index("--verify-report") + 1])
        self.assertIn("--verify-report-files", command)
        self.assertIn("--require-report-pass", command)
        self.assertEqual("c" * 40, command[command.index("--expect-commit") + 1])
    ...
    def test_saved_external_package_report_command_requires_source_trial_binding(self) -> None:
        ...
        command = release_gate.saved_external_package_report_command(report, expected_commit="d" * 40)

        self.assertEqual(str(report.resolve(strict=False)), command[command.index("--verify-report") + 1])
        self.assertIn("--verify-report-files", command)
        self.assertIn("--require-report-pass", command)
        self.assertIn("--require-trial-json", command)
        self.assertEqual("d" * 40, command[command.index("--expect-commit") + 1])
