...
class PrepareExternalL4TrialTest(unittest.TestCase):
    ...
    def test_prepare_workspace_writes_manifest_plan_and_commands(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(plan["git_commit"], stable_release[stable_release.index("--expect-commit") + 1])
            self.assertEqual(
            ...
            pre_requirement_by_name = {
                requirement["name"]: requirement for requirement in plan["pre_signoff_requirements"]
            }
            self.assertEqual(
                str((workspace / "production-evidence-audit.json").resolve()),
                pre_requirement_by_name["production_evidence_readiness_audit"]["planned_path"],
            )
            self.assertEqual(
                "verify_production_evidence_audit",
                pre_requirement_by_name["production_evidence_readiness_audit"]["verification_step"],
            )
            self.assertEqual(
                "final_production_gate",
                pre_requirement_by_name["production_evidence_readiness_audit"]["required_before"],
            )
            self.assertIn(
                str((workspace / "handoff_trial.json").resolve()),
            ...
            self.assertIn("must be generated with `--commit`, rechecked with `--expect-commit`", readme)
            self.assertIn("rechecked with `--verify-live-runs`", readme)
            ...
            self.assertIn("final_production_gate --github-workflow-verification-report", readme)
            ...
            self.assertIn("rejected instead of being treated as reviewer-ready evidence", readme)
            ...
            self.assertIn("external-evidence identity summary", readme)
            self.assertIn("acceptance-criteria count/hash", readme)
            ...
            self.assertIn("reviewer-report pair gates all pass", readme)
            self.assertIn("external-evidence identity summaries differ", readme)
            ...
            self.assertLess(
                readme.index("## verify_stable_release_report"),
                readme.index("## verify_github_workflows"),
            )
            self.assertLess(
                readme.index("## verify_github_workflows_report"),
                readme.index("## audit_production_evidence"),
            )
            self.assertLess(
                readme.index("## audit_production_evidence"),
                readme.index("## verify_production_evidence_audit"),
            )
            self.assertLess(
                readme.index("## verify_production_evidence_audit"),
            ...
            self.assertIn("必须用 `--commit` 生成，用 `--expect-commit` 复核", readme_zh)
            self.assertIn("并用 `--verify-live-runs` 重新查询远端 run", readme_zh)
            ...
            self.assertIn("final_production_gate --github-workflow-verification-report", readme_zh)
            ...
            self.assertIn("不能当作 reviewer-ready evidence", readme_zh)
            ...
            self.assertIn("external-evidence identity summary", readme_zh)
            self.assertIn("acceptance criteria 数量/hash", readme_zh)
            ...
            self.assertIn("reviewer-report pair gates 全部 PASS", readme_zh)
            self.assertIn("external-evidence identity summaries 不一致", readme_zh)
            ...
            self.assertLess(
                readme_zh.index("## verify_stable_release_report"),
                readme_zh.index("## verify_github_workflows"),
            )
            self.assertLess(
                readme_zh.index("## verify_github_workflows_report"),
                readme_zh.index("## audit_production_evidence"),
            )
            self.assertLess(
                readme_zh.index("## audit_production_evidence"),
                readme_zh.index("## verify_production_evidence_audit"),
            )
            self.assertLess(
                readme_zh.index("## verify_production_evidence_audit"),
    ...
    def test_saved_trial_plan_rejects_pre_signoff_command_binding_tampering(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            audit = payload["commands"]["audit_production_evidence"]
            audit[audit.index("--out-json") + 1] = str(workspace / "wrong-production-evidence-audit.json")
            verify_audit = payload["commands"]["verify_production_evidence_audit"]
            verify_audit[verify_audit.index("--verify-report") + 1] = str(
                workspace / "wrong-production-evidence-audit.json"
            )
    ...
    def test_saved_trial_plan_rejects_stable_release_command_binding_tampering(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            verify_release[verify_release.index("--expect-commit") + 1] = "2" * 40
        ...
        self.assertIn("commands.verify_stable_release --expect-commit must be", completed.stderr)
