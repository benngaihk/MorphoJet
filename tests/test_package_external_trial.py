...
class PackageExternalTrialTest(unittest.TestCase):
    ...
    def test_standalone_verifier_accepts_valid_package_and_writes_json(self) -> None:
        ...
        self.assertEqual("EXTERNAL_L4_EVIDENCE_PACKAGE_REVIEW", payload["evidence_scope"])
        ...
        self.assertEqual(
            "NOT_PRODUCTION_CLAIM",
            payload["input_files"]["package_artifact_manifest"]["trial_claim_status"],
        )
        self.assertEqual(
            "EXTERNAL_L4_WORKFLOW_TRIAL",
            payload["input_files"]["package_artifact_manifest"]["trial_evidence_scope"],
        )
        ...
        for readme_key in ["package_readme", "package_readme_zh"]:
            self.assertEqual(
                "external/morphojet/Objects.csv",
                payload["input_files"][readme_key]["handoff_contract"]["morphojet_objects_csv"],
            )
            self.assertEqual(
                "none",
                payload["input_files"][readme_key]["handoff_contract"]["required_object_metadata_columns"],
            )
            self.assertEqual(
                "DNA",
                payload["input_files"][readme_key]["handoff_contract"]["export[0].channels"],
            )
    ...
    def test_saved_package_verification_report_rejects_handoff_contract_tampering(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            payload = json.loads(json_out.read_text(encoding="utf-8"))
            payload["input_files"]["package_readme"]["handoff_contract"]["export[0].channels"] = "DAPI"
        ...
        self.assertEqual(1, code)
        self.assertIn(
            "input_files.package_readme.handoff_contract must match rendered manifest",
            stderr.getvalue(),
        )
    ...
    def test_saved_package_verification_report_rejects_claim_scope_tampering(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            trial_json = self.write_valid_trial(root)
            result = package_external_trial.create_package(
    ...
    def test_saved_package_verification_report_recomputes_package_trial_claim_scope(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            trial_json = self.write_valid_trial(root)
            result = package_external_trial.create_package(
        ...
        self.assertEqual(1, code)
        self.assertIn(
            "input_files.package_artifact_manifest.trial_claim_status changed after recomputing evidence package validation",
            stderr.getvalue(),
        )
        self.assertIn(
            "input_files.package_artifact_manifest.trial_evidence_scope changed after recomputing evidence package validation",
            stderr.getvalue(),
        )
        self.assertIn(
            "input_files.package_artifact_manifest.trial_final_production_signoff changed after recomputing evidence package validation",
            stderr.getvalue(),
        )
    ...
    def test_saved_package_verification_report_rejects_unbound_readiness_package_name(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            trial_json = self.write_valid_trial(root)
            result = package_external_trial.create_package(
            ...
            payload = json.loads(json_out.read_text(encoding="utf-8"))
            payload["input_files"]["package_readiness"]["package_name"] = "other-demo"
            json_out.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    ...
    def test_release_gate_rejects_package_readme_missing_readiness_context(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            trial_json = self.write_valid_trial(root)
            result = package_external_trial.create_package(
            ...
            readme_path.write_text(
                ...
                .replace(f"- readiness_workspace: `{manifest['readiness_report']['workspace']}`\n", "")
                .replace(f"- readiness_manifest: `{manifest['readiness_report']['manifest']}`\n", ""),
                encoding="utf-8",
            )
            zip_path = Path(result["zip"])
            package_external_trial.zip_directory(package_dir, zip_path)
