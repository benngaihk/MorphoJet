...
class RunProductionGateTest(unittest.TestCase):
    ...
    def test_verify_local_evidence_preflight_report_rejects_non_stable_github_tag(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            trial_json = self.write_valid_trial(root)
            package = package_external_trial.create_package(
            ...
            with contextlib.redirect_stdout(io.StringIO()):
                run_production_gate.run_local_evidence_preflight(args)
            payload = json.loads(out_json.read_text(encoding="utf-8"))
            payload["metadata"]["github_release_tag"] = "v0.1.0-rc.1"
            tag_index = payload["metadata"]["argv"].index("--github-release-tag") + 1
            payload["metadata"]["argv"][tag_index] = "v0.1.0-rc.1"
            out_json.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
        ...
        self.assertEqual(1, status)
        self.assertIn("metadata.github_release_tag must be a stable release tag", stderr.getvalue())
