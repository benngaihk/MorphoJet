...
Direct final-claim release-gate use must also keep saved reviewer evidence bound to the current final inputs: `--external-trial-verification-report` requires the same command to supply `--external-trial-json` and `--external-trial-root`; `--external-evidence-package-verification-report` requires `--external-evidence-package-dir` and `--external-trial-json`. Live GitHub release checks must stay stable and bound too: `benchmark/release_gate.py --require-production-claim --verify-github-release <tag>` rejects prerelease/RC gates unless `--github-release-kind stable` is supplied, and `--github-release-verification-report` is rejected unless the same command also supplies `--verify-github-release`.
...
Direct `benchmark/release_gate.py` final-claim runs enforce those same bindings at argument validation: saved trial reviewer reports require the current trial JSON/root, saved package reviewer reports require the current package directory and trial JSON, live `--verify-github-release` requires `--github-release-kind stable`, and `--github-release-verification-report` requires a live `--verify-github-release` tag. These checks match the final wrapper's stable non-RC release and evidence-binding requirements.
