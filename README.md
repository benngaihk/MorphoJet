...
Direct `benchmark/release_gate.py` final-claim runs that include live `--verify-github-release` enforce `--github-release-kind stable` at argument validation, and direct final-claim runs that include `--github-release-verification-report` require a live `--verify-github-release` tag so the saved stable report is not reviewed as an unbound artifact. Both checks match the final wrapper's stable non-RC release requirement.
