...
Use `benchmark/handoff/external_lab_template.json` as the starting point for real lab handoff files. External workflow trials must fill the `external_evidence` block and pass validation with `--require-external-evidence`; the trial report preserves those fields, the rendered manifest snapshot, exact manifest-declared step and artifact coverage, and one SHA-256/size provenance entry for each listed artifact for L4 review.
