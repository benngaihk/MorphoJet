...
The final production wrapper also has regression coverage for saved reviewer/verifier inputs: actual non-dry-run final commands require `--external-trial-verification-report`, `--external-evidence-package-verification-report`, and `--github-release-verification-report` together, while dry-run and local-preflight modes remain explicitly non-final.
