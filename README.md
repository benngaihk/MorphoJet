...
Latest `main` validation: commit `894de87ca8402b084abfa06db956a16ffb0806e5` has saved GitHub workflow verification for `ci.yml` and `external-l4-rehearsal.yml` on `main`, plus a refreshed full CellBinDB L3 release-gate report generated with `--require-clean-git --require-l3-provenance --run-l3`. The saved report verifies as PASS while preserving `claim_status=NOT_PRODUCTION_CLAIM` and `production_claim_status=INCOMPLETE`; the remaining blockers are still the real external L4 workflow trial, matching evidence package, saved external reviewer reports, live stable GitHub release, and saved stable-release verifier report. See [docs/VALIDATION_RESULTS.md](docs/VALIDATION_RESULTS.md) for the exact commands.
